...
class ResourceMonitor:
    ...
    def get_optimal_workers(self, use_gpu=True):
        ...
        # Each worker typically uses ~25% CPU during encoding
        cpu_workers = max(1, int(cpu_headroom / 25))
        mem_workers = max(1, int(mem_headroom / 20))  # Each worker ~20% memory
        ...
        return max(1, min(optimal, 8))  # Clamp between 1 and 8
...
class UnixAutoEdit:
    ...
    def process_all(self, folders_with_templates):
        ...
        resource_limit = self.resource_limit_var.get()
        start_time = time.time()
        active_count = [0]  # Track active workers
        ...
        def progress_fn(step_percent):
            ...
            res_status = resource_monitor.get_status(use_gpu) if auto_mode else ""
        ...
        # Dynamic worker pool - use higher max for auto-scaling
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            def submit_task():
                ...
                if not pending_tasks or self.stop_flag:
                    return False
                # Check if we can submit more (based on resource usage in auto mode)
